...
_p.add_argument("--nemo", required=True, help="snapshot .nemo a exporter")
_p.add_argument("--out", required=True, help="repertoire de deploiement")
_p.add_argument("--val", default="nemo_manifests_mixed/val_canonical.jsonl",
                help="manifeste pour la validation ONNX vs PyTorch")
_args = _p.parse_args()
NEMO_PATH = Path(_args.nemo)
DEPLOY_DIR = Path(_args.out)
...
def main():
    ...
    rows = [json.loads(l) for l in open(BASE_DIR / _args.val, encoding="utf-8")]
    random.seed(2)
